...
class QuantConf:
    ...
    share_qparam: bool = False  # concat 输入输出共享量化参数（默认 False=当前行为，与 ax_quantizer.py 对齐）
...
def get_config(config: dict[str, Any]):
    ...
    if config["input"]["dtype"] == "FP32":
        ...
        if "output" in config:  # 解耦：允许独立设 output dtype（如 silu input=U16 output=U8），与 ax_quantizer.py 对齐
