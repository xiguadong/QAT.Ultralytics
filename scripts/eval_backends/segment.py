...
class FakeTrainer:
    def __init__(self, float_model, qat_model, data_dict, device, end2end):
        ...
        self.args = argparse.Namespace(
            half=False, amp=False, compile=False, plots=False, end2end=end2end,
...
def main():
    ...
    hyp.setdefault("pose", 12.0); hyp.setdefault("kobj", 1.0)
    ...
    val_dataset = build_yolo_dataset(
        argparse.Namespace(task="segment", data=args.data, imgsz=args.imgsz, batch=val_bs,
                           ...
                           overlap_mask=overlap_mask, mask_ratio=mask_ratio, rect=False, cache=False),
        data_dict["val"], val_bs, data_dict, mode="val", rect=False, stride=gs,
    )
    val_loader = build_dataloader(val_dataset, batch=val_bs, workers=args.workers,
    ...
    cfg.update({"task": "segment", "mode": "val", "data": args.data, "imgsz": args.imgsz,
                 "batch": val_bs, "device": args.device, "workers": args.workers, "split": "val",
                 "end2end": True, "conf": 0.001, "iou": 0.7, "max_det": 300,
                 "half": False, "plots": False, "save_json": False, "save_hybrid": False,
                 "overlap_mask": overlap_mask, "mask_ratio": mask_ratio})
